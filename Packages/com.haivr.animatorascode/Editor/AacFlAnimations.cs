...
namespace AnimatorAsCode
{
    public readonly struct AacFlClip
    {
        ...
        public AacFlClip Toggling(GameObject[] gameObjectsWithNulls, bool value)
        {
            ...
            foreach (var component in defensiveObjects)
            {
                ...
                AnimationUtility.SetEditorCurve(Clip, binding, Aac.OneFrame(value ? 1f : 0f));
            }

            return this;
        }

        public AacFlClip BlendShape(
        ...
        )
        {
            ...
            AnimationUtility.SetEditorCurve(Clip, binding, Aac.OneFrame(value));

            return this;
        }

        public AacFlClip BlendShape(
            SkinnedMeshRenderer[] rendererWithNulls,
            string blendShapeName,
            float value
        )
        {
        ...
        public AacFlClip SwappingMaterial(
    ...
    public readonly struct AacFlEditClip
    {
        ...
        {
            var binding = Aac.Binding(_component, type, transform, propertyName);
        ...
        private EditorCurveBinding Internal_BindingFromComponent(
        ...
        )
        {
            return Aac.Binding(
                _component,
                anyComponent.GetType(),
                anyComponent.transform,
                propertyName
            );
        }
    }
    ...
    public class AacFlSettingCurve
    {
        ...
        public void WithOneFrame(float desiredValue)
        {
            ...
            {
                AnimationUtility.SetEditorCurve(_clip, binding, Aac.OneFrame(desiredValue));
            }
        }
        ...
        public void WithFixedSeconds(float seconds, float desiredValue)
        {
            foreach (var binding in _bindings)
            {
                AnimationUtility.SetEditorCurve(
                    _clip,
                    binding,
                    Aac.ConstantSeconds(seconds, desiredValue)
                );
            }
        }
        ...
        private void InternalWithUnit(AacFlUnit unit, Action<AacFlSettingKeyframes> action)
        {
            ...
            foreach (var binding in _bindings)
            {
                AnimationUtility.SetEditorCurve(
                    _clip,
                    binding,
                    new AnimationCurve(mutatedKeyframes.ToArray())
                );
            }
        }
    }
    ...
    public class AacFlSettingCurveColor
    {
        ...
        public void WithOneFrame(Color desiredValue)
        {
            foreach (var binding in _bindings)
            {
                AnimationUtility.SetEditorCurve(
                    _clip,
                    Aac.ToSubBinding(binding, "r"),
                    Aac.OneFrame(desiredValue.r)
                );
                AnimationUtility.SetEditorCurve(
                    _clip,
                    Aac.ToSubBinding(binding, "g"),
                    Aac.OneFrame(desiredValue.g)
                );
                AnimationUtility.SetEditorCurve(
                    _clip,
                    Aac.ToSubBinding(binding, "b"),
                    Aac.OneFrame(desiredValue.b)
                );
                AnimationUtility.SetEditorCurve(
                    _clip,
                    Aac.ToSubBinding(binding, "a"),
                    Aac.OneFrame(desiredValue.a)
                );
            }
        }
        ...
        public void WithKeyframes(AacFlUnit unit, Action<AacFlSettingKeyframesColor> action)
        {
            ...
            var builder = new AacFlSettingKeyframesColor(
                unit,
                mutatedKeyframesR,
                mutatedKeyframesG,
                mutatedKeyframesB,
            ...
            foreach (var binding in _bindings)
            {
                AnimationUtility.SetEditorCurve(
                    _clip,
                    Aac.ToSubBinding(binding, "r"),
                    new AnimationCurve(mutatedKeyframesR.ToArray())
                );
                AnimationUtility.SetEditorCurve(
                    _clip,
                    Aac.ToSubBinding(binding, "g"),
                    new AnimationCurve(mutatedKeyframesG.ToArray())
                );
                AnimationUtility.SetEditorCurve(
                    _clip,
                    Aac.ToSubBinding(binding, "b"),
                    new AnimationCurve(mutatedKeyframesB.ToArray())
                );
                AnimationUtility.SetEditorCurve(
                    _clip,
                    Aac.ToSubBinding(binding, "a"),
                    new AnimationCurve(mutatedKeyframesA.ToArray())
                );
            }
        }
    }
    ...
    public class AacFlSettingKeyframes
    {
        ...
        {
            _mutatedKeyframes.Add(
                new Keyframe(AsSeconds(timeInUnit), value, 0, float.PositiveInfinity)
            );

            return this;
        }

        public AacFlSettingKeyframes Bool(float timeInUnit, bool value)
        {
            _mutatedKeyframes.Add(
                new Keyframe(AsSeconds(timeInUnit), value ? 1 : 0, 0, float.PositiveInfinity)
            );
        ...
        public AacFlSettingKeyframes Linear(float timeInUnit, float value)
        {
            ...
            float valueStart =
                _mutatedKeyframes.Count == 0 ? value : _mutatedKeyframes.Last().value;
            float timeEnd = AsSeconds(timeInUnit);
            float timeStart = _mutatedKeyframes.Count == 0 ? value : _mutatedKeyframes.Last().time;
            float num = (float)(
                ((double)valueEnd - (double)valueStart) / ((double)timeEnd - (double)timeStart)
            );
    ...
    public class AacFlSettingKeyframesColor
    {
        ...
        public AacFlSettingKeyframesColor(
            AacFlUnit unit,
            List<Keyframe> mutatedKeyframesR,
            List<Keyframe> mutatedKeyframesG,
            List<Keyframe> mutatedKeyframesB,
            List<Keyframe> mutatedKeyframesA
        )
        {
